...
class CompleteEditSaveUseCase {
            ...
            await BaseModel.transaction(
                ...
                async (connection) => {
                    ...
                    /*==========================================
                        ...
                        The current car row is locked before
                        reading the current Display Locations
                        state.

                        This serializes Complete Edit saves
                        for the same car.
                    ==========================================*/
    ...
    /*==================================================
        ...
        Concurrency:
            ...
            1. Current car is already locked by
               Complete Edit transaction.

            2. All affected sections are locked
               before reading their ordered lists.

            3. Section locks are acquired in
               deterministic section_id order.

        Persistence:

            Existing rows are preserved whenever
            possible.

            Before applying final positions,
            existing rows are moved into a
            temporary non-conflicting range.

            This prevents UNIQUE collisions on:

                UNIQUE(section_id, sort_order)

        Important:

            - No direct final-position swap is performed.
            - No replaceForCar() is used.
            - Remove collapses the section.
            - Move is REMOVE + INSERT.
            - Out-of-range position becomes N + 1.
            - Expected state is calculated before
              persistence.
            - Actual state is validated separately
              by the caller.
    ==================================================*/
    ...
    static async reconcileDisplayLocations(
        ...
        /*==================================================
            LOAD CURRENT CAR ASSIGNMENTS

            The car row itself has already been locked
            by Complete Edit Save using FOR UPDATE.

            Therefore this read represents the current
            assignment state for this edit transaction.
        ==================================================*/
        ...
        /*==================================================
            ...
            This is required because a section from which
            the car was removed must also be collapsed.
        ==================================================*/
        ...
        /*==================================================
            LOCK ALL AFFECTED SECTIONS FIRST

            IMPORTANT:

            No section list is read for reconciliation
            until every affected section has been locked.
        ==================================================*/
        ...
        /*==================================================
            READ ALL AFFECTED SECTION STATES

            These reads happen AFTER all section locks.
        ==================================================*/
        ...
        /*==================================================
            CALCULATE FINAL SECTION STATES IN MEMORY
        ==================================================*/
        ...
        for (
            ...
            /*----------------------------------------------
                NORMALIZE REQUESTED POSITION

                Valid range:

                    1 ... remainingRows.length + 1

                Out-of-range requests become N + 1.
            ----------------------------------------------*/
            ...
            /*----------------------------------------------
                PRESERVE EXISTING ROW ID

                If this car already belongs to the section,
                reuse its existing assignment row.
            ----------------------------------------------*/
        ...
        /*==================================================
            BUILD EXPECTED CURRENT-CAR STATE

            This is calculated from the reconciliation
            result, NOT from a post-write database read.
        ==================================================*/
        ...
        for (
            ...
            if (
                ...
                expectedDisplayLocations.push({

                    section_id:
                        Number(
                            currentCarRow.section_id
                        ),

                    sort_order:
                        Number(
                            currentCarRow.sort_order
                        )
        ...
        /*==================================================
            ...
            Therefore persistence uses two phases:

                Phase 1:
                    move every existing row into
                    a temporary high range

                Phase 2:
                    apply final positions

            This makes every transition collision-safe.
        ==================================================*/
        ...
        for (
            ...
            /*
             * sectionStates currently contains the FINAL
             * state, so retrieve the original database
             * state separately.
             */
            const databaseRows =
                await CarDisplayLocation
            ...
            /*----------------------------------------------
                TEMPORARY SORT ORDER RANGE

                The temporary range is guaranteed to be
                above both:

                    original positions
                    final positions

                and therefore cannot collide with either.
            ----------------------------------------------*/

            let maximumOriginalSortOrder = 0;
            ...
            /*----------------------------------------------
                PHASE 1
                MOVE ALL EXISTING ROWS TO TEMPORARY RANGE
            ----------------------------------------------*/
                ...
                if (
                    ...
                    if (
                        ...
                        throw createError(
                            ...
                            'Display Location reconciliation encountered an invalid existing assignment.',

                            'DISPLAY_LOCATION_RECONCILIATION_STATE_ERROR'

                        );

                    }

                    finalExistingIds.add(
                        rowId
                    );
            ...
            /*----------------------------------------------
                DELETE REMOVED CURRENT-CAR ASSIGNMENT

                Any non-current row disappearing would
                indicate an unexpected reconciliation state.
            ----------------------------------------------*/
            ...
            for (
                ...
                if (
                    ...
                    throw createError(
                        ...
                        'Display Location reconciliation attempted to remove an unrelated car assignment.',
        ...
        /*==================================================
            RETURN CALCULATED EXPECTED STATE

            Never reconstruct expected state by reading
            the database after persistence.
        ==================================================*/

        return expectedDisplayLocations;
